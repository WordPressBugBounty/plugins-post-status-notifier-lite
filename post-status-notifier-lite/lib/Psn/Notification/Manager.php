...
class Psn_Notification_Manager
{
    ...
    protected function _loadServices()
    {
        ...
        do_action('psn_after_load_services', $this);
    }
    ...
    public function handlePostStatusTransition($statusAfter, $statusBefore, $post)
    {
        ...
        /**
         * Check for revision skip
         */
        if ($this->_pm->hasOption('psn_ignore_status_inherit') && $post->post_type == 'revision') {
            return;
        }
        ...
        foreach($activeRules as $rule) {
            ...
            $doMatch = apply_filters('psn_do_match', true, array(
                'rule' => $rule,
                'post' => $post,
    ...
    public function filterEmailBody($body, Psn_Notification_Service_Email $email)
    {
        ...
        if (!$this->_pm->isPremium()) {
            ...
            $body .= PHP_EOL . PHP_EOL .
                sprintf(__('This email was sent by WordPress plugin "%s". Get the premium version to remove this line (%s).', 'psn'),
                $this->_pm->getEnv()->getName(),
                $this->_pm->getConfig()->plugin->premiumUrl
                );
        }
        return $body;
    }
