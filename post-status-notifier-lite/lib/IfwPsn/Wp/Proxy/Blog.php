 ...
 * @version  $Id: Blog.php 3137090 2024-08-17 17:41:42Z worschtebrot $
 */ 
class IfwPsn_Wp_Proxy_Blog
{
     ...
     * @return float
     */
    public static function getGmtOffsetForMysql()
    {
        ...
        return $sign . $hourMysql . ':' . $minMysql;
    }
     ...
     * @return string|void
     */
    public static function getUrlWithoutScheme($path = null, $scheme = null)
    {
        $url = self::getUrl($path, $scheme);
    ...
    public static function getServerEnvironment(IfwPsn_Wp_Plugin_Manager $pm)
    {
        ...
        $mysql_server_info = IfwPsn_Wp_Proxy_Db::getObject()->db_version();

        if (function_exists('mysqli_get_client_version')) {
            $mysql_client_info = @mysqli_get_client_version();
        } else {
            $mysql_client_info = '';
        }
        ...
        // ipv6
        if (defined('AF_INET6')) {
            $ipv6 = "PHP was compiled without --disable-ipv6 option";
        } else {
            $ipv6 = "PHP was compiled with --disable-ipv6 option";
        }

        $opensslVersionText = 'unknown';
        $opensslVersionNumber = 'unknown';
        if (defined('OPENSSL_VERSION_TEXT')) {
            $opensslVersionText = OPENSSL_VERSION_TEXT;
        }
        if (defined('OPENSSL_VERSION_NUMBER')) {
            $opensslVersionNumber = OPENSSL_VERSION_NUMBER;
        }
        ...
        $context = array(
            ...
            'wp_date_format' => IfwPsn_Wp_Proxy_Blog::getDateFormat(),
            'wp_time_format' => IfwPsn_Wp_Proxy_Blog::getTimeFormat(),
            ...
            'php_hash_sha256' => in_array('sha256', hash_algos()),
        ...
        $result = $tpl->render('server_env.html.twig', $context);

        return apply_filters($pm->getAbbrLower() . '_server_env', $result);
    }
}
