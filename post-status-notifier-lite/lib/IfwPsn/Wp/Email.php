...
class IfwPsn_Wp_Email
{
    ...
    public function getAdjustedHeaders()
    {
        if ($this->_adjustedHeaders === null) {
            ...
            if ($this->hasReplyTo()) {
                array_push($adjustedHeaders, 'Reply-To: ' . $this->getReplyTo());
            }
    ...
    {
        $this->addHeader('from', self::sanitizeEmail($from));
        return $this;
    }
    ...
    protected function _sendLoopTo()
    {
        ...
        foreach ($toStack as $to) {
            do_action('ifwpsn_callback_email_loop_to', $to, $this);
    ...
    protected function _processEmail($to, $subject, $message, $headers, $attachments)
    {
        ...
        if ($process) {
            do_action('ifwpsn_callback_before_email_send', $emailParams, $this);
            $result = IfwPsn_Wp_Proxy::mail($to, $subject, $message, $headers, $attachments);
            do_action('ifwpsn_callback_after_email_send', $result, $emailParams, $this);
        }
    ...
    public function setHTML($html = true)
    {
        if (is_bool($html)) {
            if ($html == true) {
                add_filter( 'wp_mail_content_type', array($this, 'setContentTypeHTML') );
                add_action('phpmailer_init', array($this, 'phpMailerEnableHtml'));
            }
            $this->_isHTML = $html;
        }
    }
     ...
     * @return string
     */
    public function setContentTypeHTML($content_type)
    {
        return 'text/html';
    }
     ...
     * @param $phpmailer
     */
    public function phpMailerEnableHtml($phpmailer)
    {
        if ($this->_altbody != null && property_exists($phpmailer, 'AltBody')) {
