 ...
 * @version  $Id: Plugin.php 3137090 2024-08-17 17:41:42Z worschtebrot $
 */
...
class IfwPsn_Wp_Env_Plugin extends IfwPsn_Wp_Env_Abstract
{
     ...
     * @return string
     */
    protected function _determineEnvironment()
    {
        $env = 'production';
        $getenv = getenv('IFW_WP_ENV');
        if (!empty($getenv)) {
            $env = $getenv;
        } elseif (file_exists($this->_pathinfo->getDirnamePath() . 'development.txt')) {
            $env = 'development';
        }
    ...
    {
        return $this->getEnvironmet() == 'development';
    }
    ...
    {
        $upDir = wp_upload_dir();
        $dir = $upDir['basedir'] . DIRECTORY_SEPARATOR . $this->_textDomain;
        if (!file_exists($dir)) {
            mkdir($dir, 0755, true);
        }
        return $dir;
